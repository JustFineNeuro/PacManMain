...
class dPCA(BaseEstimator):
    ...
    def crossval_score(self, lams, X, trialX, mean=True):
        ...
        scores = np.zeros((self.n_trials, len(lams))) if mean else {key: np.zeros((self.n_trials, len(lams))) for key in
    ...
    def _randomized_dpca(self, X, mXs, pinvX=None):
        ...
        for key in list(mXs.keys()):
            ...
            if isinstance(self.n_components, dict):
                U, s, V = randomized_svd(np.dot(C, rX), n_components=self.n_components[key], n_iter=self.n_iter,
                                         random_state=np.random.randint(10e5))
            else:
                U, s, V = randomized_svd(np.dot(C, rX), n_components=self.n_components, n_iter=self.n_iter,
                                         random_state=np.random.randint(10e5))
    ...
    def _add_regularization(self, Y, mYs, lam, SVD=None, pre_reg=False):
        ...
        if SVD is not None:
            ...
            pregY = np.dot(regY.reshape((n_features, -1)).T, np.linalg.inv(
                np.dot(Y.reshape((n_features, -1)), Y.reshape((n_features, -1)).T) + lam ** 2 * np.eye(n_features)))
    ...
    def _add_regularization_l1(self, Y, mYs, lam):
        ...
        for key, mY in mYs.items():
            ...
            lasso.fit(reshaped_Y.T, reshaped_mY.T)  # Predict marginalized data using the original data
